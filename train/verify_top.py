...
def main(npz="weights_f56.npz", src="../src", n_img=8):
    ...
    d = np.load(npz)
    n_px = int(d["pixels"]) if "pixels" in d else 64
    W1 = d["W1"].astype(int)
    if W1.shape[1] != n_px:
        W1 = W1.T
    b1 = d["b1"].astype(int)
    ...
    imgs = (rng.random((n_img, n_px)) < 0.22).astype(int)
...
module tb;
 ...
 always #5 clk=~clk;
 reg [{n_px - 1}:0] img [0:{n_img - 1}]; reg [{bits - 1}:0] hd [0:0]; integer t,i,g;
 initial begin
  ...
  for(t=0;t<{n_img};t=t+1) begin
    for(i={n_px - 1};i>=0;i=i-1) begin ui[0]=img[t][i]; ui[1]=1'b1; @(negedge clk); end
    ui[1]=0; @(negedge clk);
  ...
  $finish; end
    ...
    for line in r.stdout.split("\n"):
        if line.startswith("S "):
            _, t, c, v = line.split()
            got[int(t), int(c)] = int(v)
            n += 1
    ok = n == gold.size and np.array_equal(got, gold)
    print(f"{mod}: {n}/{gold.size} scores, {'BIT-EXACT' if ok else 'MISMATCH'}")
    if not ok:
        print("gold\n", gold, "\ngot\n", got)
        print(r.stderr[-800:])
    return ok
