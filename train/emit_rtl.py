...
//   uio_out[2] busy
//   uio_out[{2 + cw}:3] score_idx  which class this score belongs to
...
module {module} (
    ...
    always @(posedge clk) begin
        ...
        else if (feat_shift) fidx <= (fidx == {fw}'d{n_feat - 1})
                                     ? {fw}'d0 : fidx + {fw}'d1;
    end
    wire feat_bit  = features[fidx];
    wire feat_wrap = (fidx == {fw}'d{n_feat - 1});
...
def main():
    ...
    ap.add_argument("--user", required=True, help="your GitHub username, lowercase")
    ...
    vectors = [
        ("".join(str(x) for x in reversed(im)), sc.tolist())
        for im, sc in zip(imgs, scores)
    ]
    feat_vectors = [
        ("".join(str(x) for x in reversed(im)), sc.tolist(), fe.tolist())
        for im, sc, fe in zip(imgs, scores, feats)
    ]

    tdir = Path(args.test_out)
    tdir.mkdir(parents=True, exist_ok=True)
    (tdir / "tb.v").write_text(emit_tb(module))
    (tdir / "test.py").write_text(
        emit_test_py(
            args.classes,
            bits,
            stream,
