...
def parse(t):
    ...
    return np.array([[1 if c == "#" else 0 for c in r] for r in rows], dtype=np.int32)
...
def augment(base, n, rng):
    ...
    """
    side = base.shape[0]
    out = np.zeros((n, side, side), dtype=np.int32)
    for k in range(n):
        g = base.copy()
        ...
        if r < 0.25:  # thicken
            p = np.pad(g, 1)
            g = np.maximum.reduce(
                [p[:-2, 1:-1], p[2:, 1:-1], p[1:-1, :-2], p[1:-1, 2:], g]
            )
        elif r < 0.4:  # thin
            p = np.pad(g, 1)
        ...
        # speckle
        flip = rng.random((side, side)) < 0.06
...
def main():
    ...
    for nm in names:
        if nm not in TEMPLATES:
            raise SystemExit(
                f"unknown symbol {nm!r}. available: {', '.join(TEMPLATES)}"
            )
    ...
    if args.compare:
        ...
        try:
            ...
            ftr, ltr = chipsim.subset(((xt @ W1.T + b1) >= 0).astype(np.int32), yt, cls)
            fte, lte = chipsim.subset(((xv @ W1.T + b1) >= 0).astype(np.int32), yv, cls)
